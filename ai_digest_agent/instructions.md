...
- Only include items published in the **current week** of `get_today.date` in the digest timezone (ISO week, Monday–Sunday). Discard anything outside this week and any item you cannot date.
- MANDATORY search parameters:
  - Every `web_search` call MUST pass `time_period="past_week"`.
  - Every `youtube_search` call MUST pass `upload_date="this_week"`.
- Query construction: append the long date string from `get_today.long` (e.g., `"April 11 2026"`) to every query. Do NOT invent dates.
- Each item in the output MUST have `published_at` set to a valid ISO-8601 timestamp that resolves to the current week in the digest timezone. Items with `published_at=null` are dropped automatically by `write_dashboard_files` — so verify before including.
- Publication verification — at least one of the following must be true for each item:
  ...
  - An explicit dated line on the linked page confirms a date in the current week (use `fetch_page_metadata` if needed).
...
4) **Priority sweep (MANDATORY — do this FIRST after state load):**
   Run two dedicated searches for new releases, features, and model launches from must-track companies. BOTH calls must pass `time_period="past_week"`:
   - `web_search(query="OpenAI OR Anthropic OR Claude OR Google Gemini OR DeepMind OR Meta AI new release OR launch OR feature <long>", time_period="past_week")`
   - `web_search(query="NVIDIA OR AMD OR Microsoft AI announcement OR release <long>", time_period="past_week")`
...
5) If the priority sweep yields **zero usable items from the current week** (or you still have fewer than 3 credible items), you MUST run additional targeted searches per category to find enough current-week items. Prefer primary sources. Always pass `time_period="past_week"` to `web_search` and `upload_date="this_week"` to `youtube_search`. Append `get_today.long` to the query.
   ...
   - Product launches (primary sources):
     - `web_search(query="site:openai.com (release OR launches OR introduces OR update) <long>", time_period="past_week")`
     - `web_search(query="site:anthropic.com (launch OR introduces OR release OR update) <long>", time_period="past_week")`
     - `web_search(query="(site:blog.google OR site:developers.google.com OR site:deepmind.google) (Gemini OR DeepMind OR model OR release OR launch) <long>", time_period="past_week")`
     - `web_search(query="(site:ai.meta.com OR site:about.fb.com) (Llama OR Meta AI OR model OR release OR launch) <long>", time_period="past_week")`
     - `web_search(query="(site:blogs.microsoft.com OR site:azure.microsoft.com OR site:learn.microsoft.com) (Copilot OR Azure AI OR model OR release OR launch) <long>", time_period="past_week")`
     - `web_search(query="(site:blogs.nvidia.com OR site:nvidia.com) (GPU OR CUDA OR inference OR launch OR release) <long>", time_period="past_week")`
     - `web_search(query="site:amd.com (AI OR GPU OR accelerator OR ROCm OR release OR launch) <long>", time_period="past_week")`
   - Open-source + agent tooling:
     - `web_search(query="GitHub release notes AI agent framework <long>", time_period="past_week")`
     - `web_search(query="(Claude Code OR Codex OR Cursor OR Windsurf OR Aider) update release <long>", time_period="past_week")`
     - `web_search(query="(LangChain OR LlamaIndex OR vLLM OR Ollama OR transformers) release <long>", time_period="past_week")`
   - Reporting sweep (reputable outlets; verify date on-page):
     - `web_search(query="site:techcrunch.com AI (launch OR release OR model) <long>", time_period="past_week")`
     - `web_search(query="site:theverge.com AI (launch OR release OR update) <long>", time_period="past_week")`
     - `web_search(query="site:arstechnica.com AI (launch OR release OR update) <long>", time_period="past_week")`
     - `web_search(query="site:reuters.com AI (launch OR release OR model) <long>", time_period="past_week")`
     - `web_search(query="site:bloomberg.com AI (launch OR release OR model) <long>", time_period="past_week")`
   - X discovery beyond the two must-track accounts (still requires verifiable on-page timestamp):
     - `web_search(query="site:x.com (OpenAI OR Anthropic OR Gemini OR DeepMind OR MetaAI OR Microsoft) (released OR launch OR announces) <long>", time_period="past_week")`
     - `web_search(query="site:x.com (NVIDIA OR AMD) (announces OR launch OR released) <long>", time_period="past_week")`
6) Build a ranked list. For each item, extract a concrete `published_at` timestamp (ISO-8601) from the snippet, URL, or page metadata. If the snippet does not expose a date, call `fetch_page_metadata(url=...)` and use its `published_at` if it resolves to the current week; otherwise DROP the item — do not output `null`.
7) Produce a JSON object matching the schema below.
8) Call `write_dashboard_files` with that JSON serialized as a JSON string. Note: the tool will silently drop any item whose `published_at` is missing or not in the current week (digest timezone), and will log which items were dropped.
